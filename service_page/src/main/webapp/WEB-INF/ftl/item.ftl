						...
						<div class="summary-wrap">
							...
							<div class="fl">
								<ul class="btn-choose unstyled">
									<li>
										<a href="#" @click="addToCart()" class="sui-btn  btn-danger addshopcar">加入购物车</a>
									</li>
								</ul>
							</div>
						</div>
	...
	new Vue({
		...
		methods:{
			...
			addToCart:function () {//加入购物车
				axios.get("http://localhost:8084/cart/addGoodsToCartList.do",
						{params:{itemId:this.sku.id,num:this.num}})
						.then(function (response) {
							console.log(response.data);
							if (response.data.success){
								alert(response.data.message);
								location.href="http://localhost:8084/cart.html";
							} else {
								alert(response.data.message);
							}
